...
const EditProductModal: React.FC<EditProductModalProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await updateProduct(product.product_id, productData);

      if (response.success) {
        onSuccess();
        onClose();
      } else {
        Alert.alert("Kesalahan", response.message || "Gagal mengupdate produk");
      }
    } catch (error: any) {
  ...
  // Display image: new image if selected, otherwise existing image
        ...
        <View style={[styles.modalContainer, { backgroundColor: colors.card }]}>
                ...
                <View style={styles.imagePreviewContainer}>
                  ...
                  {!imageUri && existingImageUrl && (
                    <Text style={[styles.imageNote, { color: colors.textSecondary }]}>
                      Gambar saat ini (klik X untuk hapus)
                    </Text>
                  )}
                  {imageUri && (
                    <Text style={[styles.imageNote, { color: colors.primary }]}>
                      Gambar baru (akan diupload saat save)
                    </Text>
                  )}
                </View>
              ) : (
          ...
          <View style={styles.modalFooter}>
            <TouchableOpacity
              ...
              disabled={loading}
            >
              <Text style={[styles.cancelButtonText, { color: "#ef4444" }]}>Hapus</Text>
            </TouchableOpacity>
            <TouchableOpacity
